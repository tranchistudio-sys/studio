...
function EditableName({
  ...
  // contentEditable KHÔNG kiểm soát: nếu để React render {value} làm children thì
  // mỗi keystroke React ghi đè text node → caret nhảy về đầu → nhập ngược
  // ("LONG" → "GNOL"). Chỉ đồng bộ value từ ngoài vào DOM khi KHÔNG đang gõ.
  useEffect(() => {
    if (!editable) return;
  ...
  return (
    <h1
      ref={ref}
      className={cn(className, "cursor-text rounded-md px-2 py-1 outline-none focus:ring-2 focus:ring-[#e91e63]/35")}
      contentEditable
      suppressContentEditableWarning
      spellCheck={false}
      dir="ltr"
      data-placeholder={placeholder}
      onInput={(event: FormEvent<HTMLHeadingElement>) => onChange(event.currentTarget.textContent || "")}
    />
  );
}
