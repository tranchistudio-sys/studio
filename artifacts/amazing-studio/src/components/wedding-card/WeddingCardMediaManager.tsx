...
export function WeddingCardMediaManager({ items, onPick, onRole, onSwap, onRemove, onRetry, onMove }: {
  ...
  return <section className="rounded-xl bg-white border border-[var(--wc-bt-border,#e8e0d8)] p-4 space-y-4">
    ...
    <label className="wc-bt-btn wc-bt-btn-primary w-full cursor-pointer focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-[#8f3f55]">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      {items.map((item, index) => <article key={item.id} className="relative overflow-hidden rounded-xl border border-[var(--wc-bt-border)] bg-white">
        <div className="aspect-[4/5] bg-neutral-100"><img src={getImageSrc(item.previewUrl) ?? item.previewUrl} alt={item.name} className="h-full w-full object-cover" /></div>
        ...
        <div className="p-2 space-y-2">
          ...
          {item.status === "failed" && <button type="button" onClick={() => onRetry(item.id)} className="wc-bt-btn wc-bt-btn-outline w-full py-1 text-[11px]"><RefreshCw className="h-3 w-3" />Thử lại</button>}
