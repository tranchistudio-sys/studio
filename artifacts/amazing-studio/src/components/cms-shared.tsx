...
export function LazyImage({
  ...
  // Reset khi nguồn ảnh đổi (tránh giữ trạng thái lỗi/loaded của ảnh cũ).
  useEffect(() => { setLoaded(false); setError(false); }, [finalSrc]);
  // Ảnh hỏng/404 hoặc thiếu nguồn → hiển thị placeholder đẹp thay vì khung xám thô.
  const showPlaceholder = !visible || !finalSrc || !loaded || error;
  return (
    <div ref={ref} className={`relative bg-muted ${className}`}>
      {visible && finalSrc && !error ? (
        <img
      ...
      {showPlaceholder && (
        <div className="absolute inset-0 flex items-center justify-center text-muted-foreground bg-gradient-to-br from-neutral-100 via-neutral-50 to-neutral-200">
