  ...
  return (
    ...
    >
      <div className="aspect-[3/4] relative">
        ...
        <div className={`absolute top-2 left-2 flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium ${status.bg} ${status.color}`}>
...
function ProductDrawer({ dress, categories, allProducts, defaultCategoryId, onClose, onSaved, onDeleted }: {
  ...
  // Only check code when in manual-edit mode; skip validation in auto mode
  const codeCheck = useCodeCheck(form.code, excludeId, codeEditable);
  ...
  async function handleSave() {
    ...
    if (!form.code?.trim()) { setSaveError("Vui lòng nhập mã sản phẩm"); setActiveTab(0); return; }
    // Only block on duplicate when in manual-edit mode
    if (codeEditable && codeCheck === "taken") { setSaveError("Mã sản phẩm đã được dùng, vui lòng chọn mã khác"); setActiveTab(0); return; }
    ...
    // Retry once in auto mode if backend rejects duplicate code
    let attempt = 0;
    ...
    while (attempt < 2) {
      try {
        ...
        if (!isNew) {
          ...
          if (curR.ok) {
            ...
            payload = {
              ...
              publicImageUrl: cur.publicImageUrl,
              coverImageUrl: cur.coverImageUrl,
              extraImages: cur.extraImages,
            };
          }
        }
        ...
        if (isDup && isNew && !codeEditable && attempt === 0) {
          // Auto retry with incremented code
          const fresh = regenerateAutoCode(currentForm.categoryId, true);
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[480px] bg-background shadow-2xl border-l border-border flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b bg-background sticky top-0 z-10">
          ...
          <Button size="sm" onClick={handleSave} disabled={saving || codeCheck === "taken"} className="gap-1.5 flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain pb-24">
                ...
                <div>
                  ...
                  {!codeEditable ? (
                    ...
                    <div className="flex items-center gap-2 h-10 px-3 rounded-md border border-input bg-muted/40 font-mono text-sm">
                      ...
                      {form.code && <Check className="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" />}
                    </div>
                  ) : (
                    /* Editable input */
          ...
          {activeTab === 1 && (
            <div className="p-4 space-y-4">
              {/* Ảnh bìa website (tách biệt) — hiển thị trên đầu */}
              <div className="space-y-3">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Ảnh bìa website</p>
                {form.coverImageUrl ? (
                  ...
                  <div className="text-sm text-muted-foreground">
                    Chưa có ảnh bìa. Chọn 1 ảnh từ album bên dưới hoặc upload mới.
                  </div>
                )}
                {images.length > 0 && (
              ...
              <div className="border-t pt-4 space-y-4">
                <div className="flex items-center justify-between mb-1 gap-2 flex-wrap">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                    {images.length} / 20 ảnh • {imageSelectMode ? "Bấm ảnh để chọn" : "Kéo thả để đổi thứ tự"}
                  </p>
                ...
                <p className="text-xs text-muted-foreground">
                  Kéo thả để sắp xếp ảnh trong album. Hover ảnh để xoá.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="border-t p-4 bg-background">
          <Button
            ...
            onClick={handleSave}
            disabled={saving || codeCheck === "taken"}
          >
