          ...
          {albumsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-muted-foreground text-center">
              ...
              {!search && !hasExtraFilter && (
                <Button size="sm" className="mt-4 gap-1.5" onClick={() => setEditAlbum({ status: "visible", categoryId: selectedCatId ?? null })}>
                ...
                <div className="group rounded-2xl border border-border bg-card overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    <div className="flex gap-1.5">
                      <Button size="sm" variant="outline" className="flex-1 gap-1" onClick={() => setEditAlbum(a)}>
                        <Images className="w-3.5 h-3.5" /> Ảnh ({a.photoCount})
                      </Button>
                      <Button size="sm" variant="ghost" className="px-2" onClick={() => setEditAlbum(a)}>
...
function AlbumEditorModal({
  album, cats, onClose, onSave, saving, effectiveIsAdmin, onCategoryCreated,
}: {
  ...
  // Lazy-create: tạo album ngay khi cần (lúc upload ảnh đầu tiên) nếu chưa có id.
  // Giữ nguyên API POST /cms/albums — chỉ đổi THỜI ĐIỂM tạo (sớm hơn, không cần bấm Lưu trước).
  async function ensureAlbumId(): Promise<number | null> {
    if (draft.id) return draft.id;
    if (!draft.name?.trim()) {
      toast({ title: "Nhập tên album trước khi upload ảnh", variant: "destructive" });
      return null;
    }
    if (creatingAlbum) return null;
    setCreatingAlbum(true);
    try {
      const r = await fetch(`${CMS_BASE}/api/cms/albums`, {
        method: "POST", headers: authHeaders(), body: JSON.stringify(draft),
      });
      const row = await r.json().catch(() => ({}));
      if (!r.ok || !row?.id) {
        toast({ title: "Lỗi tạo album", description: row?.error, variant: "destructive" });
        return null;
      }
      setDraft(d => ({ ...d, ...row }));
      qc.invalidateQueries({ queryKey: ["cms-albums"] });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4" onClick={onClose}>
      <div
        className="bg-background rounded-2xl shadow-xl w-full max-h-[95vh] flex flex-col max-w-3xl"
      ...
      >
        ...
        <div className="p-5 space-y-3 overflow-y-auto flex-1">
          ...
          {/* ── Ảnh album — cùng màn hình; album tự tạo khi upload, không cần lưu trước ── */}
          <div className="border-t pt-4 mt-1">
            <p className="text-sm font-semibold mb-3 flex items-center gap-1.5">
              <Images className="w-4 h-4" /> Ảnh album
            </p>
            {draft.id ? (
              <AlbumPhotosTab
              ...
              <NewAlbumPhotoDropzone
                disabled={!draft.name?.trim() || creatingAlbum}
                ensureAlbumId={ensureAlbumId}
                draftCover={draft.coverImageUrl ?? null}
                onCreated={patch => setDraft(d => ({ ...d, ...patch }))}
              />
            )}
          </div>
        </div>
...
// Dropzone cho album CHƯA có id: upload ảnh/video → tự tạo album (ensureAlbumId) →
// lưu ảnh → ảnh đầu tự làm bìa. Sau khi có id, parent chuyển sang AlbumPhotosTab đầy đủ.
function NewAlbumPhotoDropzone({
  disabled, ensureAlbumId, draftCover, onCreated,
}: {
  disabled: boolean;
  ensureAlbumId: () => Promise<number | null>;
  draftCover: string | null;
  onCreated: (patch: Partial<Album>) => void;
}) {
  const qc = useQueryClient();
  const { toast } = useToast();
  ...
  async function handleUploaded(imgs: UploadedImage[]) {
    if (!imgs.length) return;
    setBusy(true);
    try {
      const id = await ensureAlbumId();
      if (!id) return;
      const r = await fetch(`${CMS_BASE}/api/cms/albums/${id}/photos`, {
        method: "POST", headers: authHeaders(),
      ...
      if (!r.ok) {
        toast({ title: "Lỗi lưu ảnh", description: (inserted as { error?: string })?.error, variant: "destructive" });
        return;
      }
      // Ảnh đầu tiên tự làm ảnh bìa nếu album chưa có bìa (dùng API cover sẵn có)
      let coverImageUrl = draftCover;
      const first = Array.isArray(inserted) ? inserted[0] : null;
      if (!coverImageUrl && first?.id) {
        const cr = await fetch(`${CMS_BASE}/api/cms/albums/${id}/cover`, {
          method: "POST", headers: authHeaders(), body: JSON.stringify({ photoId: first.id }),
        });
        const cj = await cr.json().catch(() => ({}));
        if (cr.ok && cj.coverImageUrl) coverImageUrl = cj.coverImageUrl;
      }
      onCreated({ id, ...(coverImageUrl ? { coverImageUrl } : {}) });
      qc.invalidateQueries({ queryKey: ["cms-photos", id] });
      qc.invalidateQueries({ queryKey: ["cms-albums"] });
      qc.invalidateQueries({ queryKey: ["public-gallery-albums"] });
      qc.invalidateQueries({ queryKey: ["public-gallery-categories"] });
      toast({ title: "Đã tạo album & thêm ảnh", description: `${imgs.length} ảnh` });
  ...
  return (
    <div className="space-y-3">
      {disabled && (
        <p className="text-xs text-amber-600">Nhập <b>tên album</b> phía trên, rồi upload ảnh — album sẽ tự tạo.</p>
      )}
      <div className={disabled ? "opacity-50 pointer-events-none select-none" : ""}>
        <MultiImageUploader
          multiple
          useQueue={false}
          label="Kéo thả / dán Ctrl+V / bấm để upload nhiều ảnh — album tự tạo"
          onUploaded={handleUploaded}
        />
        <div className="mt-3">
          <VideoUploaderButton caps={caps} onUploaded={handleUploaded} />
        </div>
      </div>
      {busy && (
        <p className="text-xs text-muted-foreground flex items-center gap-1.5">
          <Loader2 className="w-3 h-3 animate-spin" /> Đang tạo album & lưu ảnh…
        </p>
      )}
    </div>
  );
...
function AlbumPhotosTab({
  ...
  const addPhotos = useMutation({
    mutationFn: async (imgs: UploadedImage[]) => {
      ...
      const inserted = await r.json().catch(() => []);
      if (!r.ok) throw new Error((inserted as { error?: string })?.error ?? "Lỗi lưu ảnh");
      // Ảnh đầu tiên tự làm ảnh bìa nếu album chưa có bìa
      const first = Array.isArray(inserted) ? inserted[0] : null;
      if (!currentCover && first?.id) {
        const cr = await fetch(`${CMS_BASE}/api/cms/albums/${album.id}/cover`, {
          method: "POST", headers: authHeaders(), body: JSON.stringify({ photoId: first.id }),
        });
        const cj = await cr.json().catch(() => ({}));
        if (cr.ok && cj.coverImageUrl) { setCurrentCover(cj.coverImageUrl); onCoverChanged?.(cj.coverImageUrl); }
      }
      return imgs.length;
    },
    onSuccess: (count) => {
  ...
  return (
    <div className="p-4 sm:p-5 space-y-4">
      ...
      <MultiImageUploader
        multiple
        useQueue={false}
        label="Kéo thả / dán Ctrl+V / bấm để upload nhiều ảnh cùng lúc"
