...
export default function RentalDetailPage() {
  ...
  // Sản phẩm không tồn tại (slug sai / đã gỡ) → tự quay về danh sách /cho-thue-do.
  const notFound = !isLoading && ((error as Error)?.message === "not_found" || (!dress && !!slug));
  useEffect(() => {
    if (!notFound) return;
    const t = setTimeout(() => setLocation(`${BASE}/cho-thue-do`), 2500);
    return () => clearTimeout(t);
  }, [notFound, BASE, setLocation]);
      ...
      <div className="min-h-[50vh] flex items-center justify-center text-center px-6">
        <div>
          ...
          <p className="text-neutral-600 text-sm mb-6">
            ...
            <br />
            <span className="text-neutral-400 text-xs">Đang đưa bạn về danh sách đồ cho thuê...</span>
          </p>
          <button
            type="button"
