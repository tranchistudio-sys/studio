...
while IFS= read -r service; do
  cid=$(docker compose -f "$DEPLOY_COMPOSE_FILE" ps -q "$service")
  [ -n "$cid" ] || continue
  if docker inspect "$cid" --format '{{range .Config.Env}}{{println .}}{{end}}' | grep -q '^DATABASE_URL='; then
    API_SERVICE="$service"
    API_CID="$cid"
    break
  fi
done < <(docker compose -f "$DEPLOY_COMPOSE_FILE" config --services)
[ -n "$API_CID" ] || die "Không tìm thấy service API có DATABASE_URL"
API_NETWORK=$(docker inspect "$API_CID" --format '{{range $name, $_ := .NetworkSettings.Networks}}{{println $name}}{{end}}' | head -n 1)
[ -n "$API_NETWORK" ] || die "Không xác định được Docker network production"
echo "[platform-auth] Dùng service API: $API_SERVICE"

read_api_env() {
  docker inspect "$API_CID" --format '{{range .Config.Env}}{{println .}}{{end}}' \
    | sed -n "s/^$1=//p" | head -n 1
}

DATABASE_URL=$(read_api_env DATABASE_URL)
SESSION_SECRET=$(read_api_env SESSION_SECRET)
...
url_for_database() {
  docker exec -e SOURCE_DATABASE_URL="$DATABASE_URL" -e TARGET_DATABASE="$1" "$API_CID" \
...
docker run --rm --network "$API_NETWORK" -e TARGET_URL="$DATABASE_URL" "$PG_IMAGE" \
...
COUNTS=$(docker run --rm --network "$API_NETWORK" -e TARGET_URL="$DATABASE_URL" "$PG_IMAGE" sh -c \
...
BUSINESS_TABLES=$(docker run --rm --network "$API_NETWORK" \
