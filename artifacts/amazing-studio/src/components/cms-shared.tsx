...
export function MultiImageUploader({
  ...
  const [queue, setQueue] = useState<Array<{ name: string; progress: number; error?: string }>>([]);
  ...
  const handleFiles = useCallback(async (files: File[]) => {
    ...
    if (useQueue) {
      const ids: string[] = [];
      setQueue(list.map(f => ({ name: f.name || "ảnh dán", progress: 0 })));
      for (let i = 0; i < list.length; i++) {
        const id = await uploadQueueStore.enqueue(list[i], attach);
        ids.push(id);
        const idx = i;
        const unsub = uploadQueueStore.subscribe((jobs) => {
          ...
          if (j.status === "uploading" || j.status === "pending") {
            setQueue(q => q.map((x, qi) => qi === idx ? { ...x, progress: j.progress || 5 } : x));
          }
          if (j.status === "uploaded" && j.objectPath) {
            setQueue(q => q.map((x, qi) => qi === idx ? { ...x, progress: 100 } : x));
            if (j.attach?.dressId) {
          ...
          if (j.status === "failed") {
            setQueue(q => q.map((x, qi) => qi === idx ? { ...x, error: j.error ?? "Lỗi upload" } : x));
            unsub();
          }
        });
      }
