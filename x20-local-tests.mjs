...
const dir = await mkdtemp(path.join(os.tmpdir(), "x20-secret-")); const store = createSecretStore(dir); await store.save("fake-key"); assert.equal(await store.configured(), true); assert.equal(await store.active(), true); await store.rotate("fake-key-2"); assert.equal(await store.read(), "fake-key-2"); await store.deactivate(); assert.equal(await store.active(), false); await store.activate(); assert.equal(await store.active(), true); assert.equal((await stat(path.join(dir, "x20-api-key"))).mode & 0o777, 0o600); await store.delete(); assert.equal(await store.configured(), false); assert.equal(redact("Authorization: Bearer secret"), "Authorization: Bearer <REDACTED>");
